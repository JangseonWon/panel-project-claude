...
import com.gcgenome.lims.dao.AbstractElasticsearchDAO;
import com.gcgenome.lims.dao.SearchableElasticsearch;
...
@Repository
public class AnalysisSnvDao extends AbstractElasticsearchDAO implements SearchableElasticsearch {
	public AnalysisSnvDao(ElasticsearchRestTemplate em) {
	...
	public void where(CriteriaBuilderAbstract cb, QueryServerside.Filter filter) {
		switch(filter.key().toLowerCase()) {
			...
			case "hgvsc":	cb.and(criteria().get("hgvsc_in_mane").contains(filter.value().trim())); break;
			case "hgvsp":	cb.and(criteria().get("hgvsp_in_mane").contains(filter.value().trim())); break;
			case "snv_exact" : cb.and(criteria().get("snv").is(filter.value().trim())); break;
			case "genes":	{
			...
			case "codon":
			case "exon":
			case "exon_in_mane":	cb.and(criteria().get(filter.key().trim().toLowerCase()).contains(escapeSlash(filter.value().trim()))); break;
			// Slash Analyzer 는 검색 값으로 사용시 작동하지 않는다(특수문자 Analyzer는 작동함).
