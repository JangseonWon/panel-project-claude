...
import com.greencross.lims.test.geneplus.TestInfo;
...
public class GenePlusExpandElement extends HTMLElementBuilder<HTMLDivElement, GenePlusExpandElement> implements ExpandElement<HTMLDivElement> {
	public static GenePlusExpandElement build(String id, long sample, String service) {
		return new GenePlusExpandElement(div(), id, sample, service);
	}
	...
	// private final ChipElementCheckable hasMulti = ChipElement.check("Multimapping");
	private final ChipElementCheckable panelOrGene = ChipElement.check("").style("display: none");
	private final ChipElementCheckable tagTier1 = ChipElement.check("Tier1").value(true);
	private final ChipElementCheckable tagTier2 = ChipElement.check("Tier2").value(true);
	private final ChipSetElement tagSet = ChipSetElement.filters(panelOrGene, hasFiltered, tagTier1, tagTier2, mitochondrial);
	...
	private final Set<String> tier1;
	private final Set<String> tier2;
	...
	private GenePlusExpandElement(HtmlContentBuilder<HTMLDivElement> e, String id, long sample, String service) {
		super(e);
		this.id = id;
		this.sample = sample;
		this.service = service;
		_this = e.css("work").style("height: 100vh;");
		test = Stream.concat(Arrays.stream(TestInfo.TESTS_ETC), Arrays.stream(TestInfo.TESTS_BRCA))
					 .filter(t->t.code().equals(service)).findAny().orElse(null);
		tier1 = Arrays.stream(new String[] {test.gene()}).collect(Collectors.toSet());
		tier2 = Arrays.stream(test.tier2()).collect(Collectors.toSet());
		if(tier2.isEmpty()) tagTier2.value(false).element().remove();
		...
		tagTier1.on(EventType.click, evt -> update());
		tagTier2.on(EventType.click, evt -> update());
	...
	private void updateVcfTable(Analysis2 dto) {
		...
		if(tagTier1.value()) list.addAll(tier1);
		if(tagTier2.value()) list.addAll(tier2);
		if(!list.isEmpty()) {
		...
		SnvApi.snvs(sample, service, dto.batch(), dto.row(), query.page((int) page.page()).limit(page.show()).sortBy("snv").asc(true).filters(filters.stream().toArray(Query.Filter[]::new)))
			  .then(objs->{
				  ...
				  for(Object obj: objs.content()) {
					  ...
					  if(tier1.contains(gene)) map.set("tier", "Tier1");
					  else if(tier2.contains(gene)) map.set("tier", "Tier2");
					  else map.set("tier", "-");
				  }
	...
	@Override
	public GenePlusExpandElement that() {
		return this;
	}
