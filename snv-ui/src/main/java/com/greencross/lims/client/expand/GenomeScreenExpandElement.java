...
import com.greencross.lims.test.genomescreen.TestInfo;
...
public class GenomeScreenExpandElement extends HTMLElementBuilder<HTMLDivElement, GenomeScreenExpandElement> implements ExpandElement<HTMLDivElement> {
	...
	private final ChipElementCheckable targets = ChipElement.check("Target").value(true);
	private final ChipSetElement tagSet = ChipSetElement.filters(hasFiltered, targets);
	...
	private GenomeScreenExpandElement(HtmlContentBuilder<HTMLDivElement> e, String id, long sample, String service) {
		super(e);
		this.id = id;
		this.sample = sample;
		...
		targets.on(EventType.click, evt -> update());
	...
	private void updateVcfTable(Analysis2 dto) {
		...
		if(targets.value()) {
			...
			Arrays.stream(test.genes()).forEach(list::add);
