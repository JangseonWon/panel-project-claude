...
public class SingleGeneExpandElement extends HTMLElementBuilder<HTMLDivElement, SingleGeneExpandElement> implements ExpandElement<HTMLDivElement> {
	public static SingleGeneExpandElement build(String id, long sample, String service) {
		return new SingleGeneExpandElement(div(), id, sample, service);
	}
	...
	private final ChipSetElement tagSet = ChipSetElement.filters(panelOrGene, hasFiltered, core, addendum, mitochondrial);
	...
	private Analysis2 currentTarget;

	private SingleGeneExpandElement(HtmlContentBuilder<HTMLDivElement> e, String id, long sample, String service) {
		super(e);
		this.id = id;
		this.sample = sample;
		...
		iptAnalysis.onValueChange(evt->{
			currentTarget = analysis.get(evt.value());
			updateVcfTable(currentTarget);
		});
				  ...
				  .then(as->{
					  ...
					  if(as==null || as.length<=0) notAnalyzed.element().style.display = null;
						  ...
						  if(currentTarget == null){
							  ...
							  currentTarget = analysis.get(iptAnalysis.value());
						  }
						  updateVcfTable(currentTarget);
					  }
					  return null;
	...
	@Override
	public SingleGeneExpandElement that() {
		return this;
	}
