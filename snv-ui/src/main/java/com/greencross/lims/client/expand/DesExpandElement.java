...
import com.greencross.lims.test.des.TestInfo;
...
public class DesExpandElement extends HTMLElementBuilder<HTMLDivElement, DesExpandElement> implements ExpandElement<HTMLDivElement> {
	public static DesExpandElement build(String id, long sample, String service) {
		return new DesExpandElement(div(), id, sample, service);
	}
	...
	private final ChipSetElement tagSet = ChipSetElement.filters(hasFiltered, incidental, mitochondrial);
	...
	private DesExpandElement(HtmlContentBuilder<HTMLDivElement> e, String id, long sample, String service) {
		super(e);
		this.id = id;
		this.sample = sample;
		this.service = service;
		_this = e.css("work").style("height: 100vh;");
		ifs = Arrays.stream(TestInfo.GENE_INCIDENTAL_FINDINGS).collect(Collectors.toSet());
