...
public class PanelHeaderEnUs extends PanelHeader {
	...
	public PanelHeaderEnUs(PanelTemplate template) {
		super(template);
	}
	@Override
	public Painter<PanelTemplate, PanelDto> initialize() {
		return (stream, template, dto) -> {
			...
			styleTitle = new TextStyle().color(resource.colorSecondary()).fonts(resource.fontTitle(), resource.fontDefault()).fontSize(17).paragraph(false);
