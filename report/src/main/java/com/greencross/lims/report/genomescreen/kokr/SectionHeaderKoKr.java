...
public class SectionHeaderKoKr<R extends GenomeScreenResourceKoKr> implements Painter<GenomeScreenTemplateKoKr<R>, GenomeScreenDto> {
	...
	@Override
	public PDPageContentStreamPageAccessible paint(PDPageContentStreamPageAccessible stream, GenomeScreenTemplateKoKr<R> template, GenomeScreenDto dto) throws IOException {
		...
		GenomeScreenResourceKoKr resource = template.resource();
		TextStyle styleTitle = new TextStyle().color(resource.colorText()).fonts(resource.fontHeader(), resource.fontText(), resource.fontDefault()).fontSize(8).justify(true).paragraph(false);
		...
		stream.paragraph(BOX1_START_X+5, y - MARGIN - heightRow/2, BOX_WIDTH - 10, JUSTIFY, MIDDLE, new TextBlock(styleTitle, template.lblMedicalInstitution()));
		stream.paragraph(BOX1_START_X+5, y - MARGIN - heightRow/2*3, BOX_WIDTH - 10, JUSTIFY, MIDDLE, new TextBlock(styleTitle, template.lblPatientName()));
		stream.paragraph(BOX1_START_X+5, y - MARGIN - heightRow/2*5, BOX_WIDTH - 10, JUSTIFY, MIDDLE, new TextBlock(styleTitle, template.lblMedicalRecordNumber()));
		stream.paragraph(BOX1_START_X+5, y - MARGIN - heightRow/2*7, BOX_WIDTH - 10, JUSTIFY, MIDDLE, new TextBlock(styleTitle, template.lblPatientInfo()));
		stream.paragraph(BOX2_START_X+5, y - MARGIN - heightRow/2, BOX_WIDTH - 10, JUSTIFY, MIDDLE, new TextBlock(styleTitle, template.lblRequestNumber()));
		stream.paragraph(BOX2_START_X+5, y - MARGIN - heightRow/2*3, BOX_WIDTH - 10, JUSTIFY, MIDDLE, new TextBlock(styleTitle, template.lblAgeSex()));
		stream.paragraph(BOX2_START_X+5, y - MARGIN - heightRow/2*5, BOX_WIDTH - 10, JUSTIFY, MIDDLE, new TextBlock(styleTitle, template.lblSpecimenTypeDate()));
		stream.paragraph(BOX2_START_X+5, y - MARGIN - heightRow/2*7, BOX_WIDTH - 10, JUSTIFY, MIDDLE, new TextBlock(styleTitle, template.lblReceiptReportDate()));
		...
		stream.paragraph(BOX1_END_X+2, y - MARGIN - heightRow/2*5, valueWidth, MIDDLE, new TextBlock(styleValue, Util.dashIfEmpty(dto.medicalRecordNumber())));
		stream.paragraph(BOX1_END_X+2, y - MARGIN - heightRow/2*7, valueWidth, MIDDLE, new TextBlock(styleValue, Util.dashIfEmpty(dto.patientInfo())));
		...
		stream.paragraph(BOX2_END_X+2, y - MARGIN - heightRow/2*3, valueWidth, MIDDLE, new TextBlock(styleValue, Util.dashIfEmpty(template.age(dto.age(), dto.birthDate(), dto.collectionDate())) + " / " + Util.dashIfEmpty(template.sex(dto.sex()))));
		stream.paragraph(BOX2_END_X+2, y - MARGIN - heightRow/2*5, valueWidth, MIDDLE, new TextBlock(styleValue,  "WB / " + Util.dashIfEmpty(template.date(dto.collectionDate()))));
		stream.paragraph(BOX2_END_X+2, y - MARGIN - heightRow/2*7, valueWidth, MIDDLE, new TextBlock(styleValue, Util.dashIfEmpty(template.date(dto.receiptDate())) + " / " + Util.dashIfEmpty(template.date(dto.reportDate()))));
