...
public class SectionClinicalMeaningsRiskScreen implements Painter<RiskScreenTemplate<RiskScreenResource>, GenomeScreenWithRiskScreenDto> {
	...
	public SectionClinicalMeaningsRiskScreen(Painter<GenomeScreenTemplate<GenomeScreenResource>, GenomeScreenDto> newPage) {
		this.newPage = newPage.and((s, t, d)->header(s, t, (GenomeScreenWithRiskScreenDto)d));
	}
	@Override
	public PDPageContentStreamPageAccessible paint(PDPageContentStreamPageAccessible stream, RiskScreenTemplate<RiskScreenResource> template, GenomeScreenWithRiskScreenDto dto) throws IOException {
		stream = header(stream, (GenomeScreenTemplate)template, dto);
		stream = tier1(stream, template);
		stream = tier2(stream, template, dto);
		stream = tier3(stream, template, dto);
		return stream;
	}
	private PDPageContentStreamPageAccessible header(PDPageContentStreamPageAccessible stream, GenomeScreenTemplate<GenomeScreenResource> template, GenomeScreenWithRiskScreenDto dto) throws IOException {
		...
		else if(mode == 1) y = tier1Header(stream, (RiskScreenTemplate)template).cursorY();
		else if(mode == 2) y = tier2Header(stream, (RiskScreenTemplate)template).cursorY();
		else if(mode == 3) y = tier3Header(stream, (RiskScreenTemplate)template, dto).cursorY();
		stream.restoreGraphicsState();
		return stream.cursorY(y);
	}
	private PDPageContentStreamPageAccessible tier1Header(PDPageContentStreamPageAccessible stream, RiskScreenTemplate<RiskScreenResource> template) throws IOException {
		mode = 1;
		stream.saveGraphicsState();
		var resource = template.resource();
		var y = stream.cursorY();
		TextStyle styleHeader = new TextStyle().color(Color.WHITE).fonts(resource.fontHeader(), resource.fontDefault()).fontSize(8);
		TextBlock block = new TextBlock(styleHeader, template.lblClinicalMeanings(RiskScreenTemplate.Tier.Tier1));
		...
		stream.paragraph(65, y-10, 487, block);
		y -= height;
		stream.restoreGraphicsState();
		return stream.cursorY(y);
	}
	...
	private PDPageContentStreamPageAccessible tier1(PDPageContentStreamPageAccessible stream, RiskScreenTemplate<RiskScreenResource> template) throws IOException {
		List<RiskScreenTemplate.GeneRiskScreen> genes = Arrays.stream(template.riskscreens())
															  .map(d->d.subs()).flatMap(Arrays::stream)
															  .map(d->d.genes()).flatMap(Arrays::stream)
															  .filter(g->g.tier() == RiskScreenTemplate.Tier.Tier1)
															  .distinct().sorted()
	...
	private PDPageContentStreamPageAccessible tier2Header(PDPageContentStreamPageAccessible stream, RiskScreenTemplate<RiskScreenResource> template) throws IOException {
		mode = 2;
		stream.saveGraphicsState();
		var resource = template.resource();
		var y = stream.cursorY();
		TextStyle styleHeader = new TextStyle().color(Color.WHITE).fonts(resource.fontHeader(), resource.fontDefault()).fontSize(8);
		TextBlock block = new TextBlock(styleHeader, template.lblClinicalMeanings(RiskScreenTemplate.Tier.Tier2));
		var height = stream.height(487, block) + 5;
		stream.setNonStrokingColor(resource.colorPrimary()).rect(50, y, 497, height).fill();
		stream.paragraph(55, y-10, 10, new TextBlock(styleHeader, "▶"));
		stream.paragraph(65, y-10, 487, block);
	...
	private PDPageContentStreamPageAccessible tier2(PDPageContentStreamPageAccessible stream, RiskScreenTemplate<RiskScreenResource> template, GenomeScreenDto dto) throws IOException {
		List<RiskScreenTemplate.GeneRiskScreen> genes = Arrays.stream(template.riskscreens())
															  .map(d->d.subs()).flatMap(Arrays::stream)
															  .map(d->d.genes()).flatMap(Arrays::stream)
															  .filter(g->g.tier() == RiskScreenTemplate.Tier.Tier2)
															  .distinct().sorted()
															  .collect(Collectors.toList());
		if(genes.isEmpty()) return stream;
		mode = 2;
	...
	private PDPageContentStreamPageAccessible tier2(PDPageContentStreamPageAccessible stream, RiskScreenTemplate<RiskScreenResource> template, GenomeScreenDto dto, RiskScreenTemplate.GeneRiskScreen gene) throws IOException {
		var resource = template.resource();
		var y = stream.cursorY();
		TextStyle styleHeader = new TextStyle().color(resource.colorPrimary()).fonts(resource.fontScientific(), resource.fontDefault()).fontSize(8);
		...
		if(y - height < 110) {
			stream = newPage.paint(stream, (GenomeScreenTemplate)template, dto);
			y = stream.cursorY();
		}
	...
	private PDPageContentStreamPageAccessible tier3Header(PDPageContentStreamPageAccessible stream, RiskScreenTemplate<RiskScreenResource> template, GenomeScreenDto dto) throws IOException {
		mode = 3;
		var resource = template.resource();
		var y = stream.cursorY();
		TextStyle styleHeader = new TextStyle().color(Color.WHITE).fonts(resource.fontHeader(), resource.fontDefault()).fontSize(8);
		TextBlock block = new TextBlock(styleHeader, template.lblClinicalMeanings(RiskScreenTemplate.Tier.Tier3));
		var height = stream.height(487, block) + 5;
		List<RiskScreenTemplate.GeneRiskScreen> genes = Arrays.stream(template.riskscreens())
				.map(RiskScreenTemplate.DiseaseRiskScreen::subs).flatMap(Arrays::stream)
				.map(RiskScreenTemplate.DiseaseSubRiskScreen::genes).flatMap(Arrays::stream)
				.filter(g->g.tier() == RiskScreenTemplate.Tier.Tier3)
				.distinct().sorted()
				.collect(Collectors.toList());
	...
	private PDPageContentStreamPageAccessible tier3(PDPageContentStreamPageAccessible stream, RiskScreenTemplate<RiskScreenResource> template, GenomeScreenDto dto) throws IOException {
		List<RiskScreenTemplate.GeneRiskScreen> genes = Arrays.stream(template.riskscreens())
															  .map(RiskScreenTemplate.DiseaseRiskScreen::subs).flatMap(Arrays::stream)
															  .map(RiskScreenTemplate.DiseaseSubRiskScreen::genes).flatMap(Arrays::stream)
															  .filter(g->g.tier() == RiskScreenTemplate.Tier.Tier3)
