...
public class GenomeScreenTemplateN090KoKr extends GenomeScreenTemplateKoKr<GenomeScreenResourceN090KoKr> implements GenomeScreenTemplateN090<GenomeScreenResourceN090KoKr> {
	...
	public GenomeScreenTemplateN090KoKr(GenomeScreenResourceN090KoKr resource, TestInfo testInfo) {
		super(testInfo);
	...
	private String diseaseName(DiseaseN090 disease) {
		switch (disease) {
			...
			case 유방암_감수성:				return "유방암 감수성";
			case 난소암_감수성:				return "난소암 감수성";
			case 전립선암_감수성:				return "전립선암 감수성";
			case 리_프라우메니_증후군:			return "리-프라우메니증후군";
			case 유전성_미만성_위암:			return "유전성 미만성 위암";
	...
	private String diseaseInfo(DiseaseN090 disease) {
		switch (disease) {
			...
			case 유방암_감수성:				return "이상 발견 시 유전성 유방암 발생률을 높이는 BRCA1, BRCA2 외 유전자(남성에서 병원성 변이 발견 시 여성 직계 가족에 대한 가족 검사를 시행해 보는 것이 좋습니다.)";
			case 난소암_감수성:				return "이상 발견 시 유전성 난소암 발생률을 높이는 BRCA1, BRCA2 외 유전자(남성에서 병원성 변이 발견 시 여성 직계 가족에 대한 가족 검사를 시행해 보는 것이 좋습니다.)";
			case 전립선암_감수성:				return "이상 발견 시 유전성 전립선암 발생률을 높이는 BRCA1, BRCA2 외 유전자(여성에서 병원성 변이 발견 시, 유방암 감수성 혹은 린치증후군 결과에 대한 해석을 참조하시기 바랍니다.)";
			case 리_프라우메니_증후군:			return "종양억제유전자인 TP53의 이상으로 인해 상염색체 우성으로 유전되는 가족성 암 질환";
			case 유전성_미만성_위암:			return "CDH1 유전자의 이상으로 인해 상염색체 우성으로 유전되는 위 점막 아래로 생성되는 위암이 발생하는 질환";
	...
	private String diseaseSubName(DiseaseSubN090 diseaseSub) {
		switch (diseaseSub) {
			...
			case 유방암_감수성:						return "유방암";
			case 난소암_감수성:						return "난소암";
			case 전립선암_감수성:						return "전립선암";
	...
	public String diseaseSubInfo(DiseaseSub diseaseSub) {
		if(diseaseSub instanceof DiseaseSubN090) switch ((DiseaseSubN090) diseaseSub) {
			...
			case 유방암_감수성:						return "유방암의 5-20%정도는 부모로부터 물려받아 태어날 때부터 가지고 있는 유전자 이상에 의해 발생합니다. 유전성 유방암의 대부분은 BRCA1과 BRCA2 유전자의 병원성 변이가 주요 원인이나 다른 유전자들의 변이에 의해서 유방암 발생률이 증가하기도 하며, 이를 유방암 감수성 유전자라고 합니다.";
			case 난소암_감수성:						return "유전성 난소암의 대부분은 BRCA1과 BRCA2 유전자의 병원성 변이가 주요 원인이나 다른 유전자들의 변이에 의해서 유방암 발생률이 증가하기도 하며, 이를 난소암 감수성 유전자라고 합니다.";
			case 전립선암_감수성:						return "전립선암 환자의 9% 정도는 유전에 의해서 발생합니다. 유전성 전립선암은 DNA 복구 유전자의 병원성 변이가 주요 원인이며, 이를 전립선암 감수성 유전자라고 합니다. 유전성 전립선암은 일반 전립선암에 비해 비교적 이른 시기에 발생하며, 특히 55세 이전에 많이 발생합니다.";
			...
			case 위암:								return "미만성 위암은 뚜렷한 덩어리를 형성하지 않고 작은 위암 세포가 점막 쪽으로 파고드는 형태로 생성되는 위암입니다. 증상이 없고 진행속도가 빠른 것이 특징으로 3, 4기에 발견이 되는 경우가 많습니다. 또한 혈관이나 림프절 등 다른 부위로의 전이가 빠른 암입니다. 유전성 미만성 위암은 상염색체 우성으로 유전되는 질환으로 CDH1 유전자의 병원성 변이가 원인입니다. 발병 평균 연령은 38세로 발병 연령이 빠른 편입니다.";
	...
	public String diseaseSubReference(DiseaseSub diseaseSub) {
		if(diseaseSub instanceof DiseaseSubN090) switch ((DiseaseSubN090) diseaseSub) {
			...
			case 유방암_감수성:					return "03. 예방 및 치료 page14, 17~18 참조";
			case 난소암_감수성:					return "03. 예방 및 치료 page15, 17~18 참조";
			case 전립선암_감수성:					return "03. 예방 및 치료 page16~18 참조";
			case 리_프라우메니_증후군:				return "03. 예방 및 치료 page19~21 참조";
			case 위암:							return "03. 예방 및 치료 page22~23 참조";
			case 포이츠_제거스_증후군:				return "03. 예방 및 치료 page24~25 참조";
			case 린치_증후군:						return "03. 예방 및 치료 page26~29 참조";
			case 가족성_선종성_용종증:				return "03. 예방 및 치료 page30~31 참조";
			case MUTYH_연관_용종증:				return "03. 예방 및 치료 page32~33 참조";
			case 연소성_용종증_증후군:				return "03. 예방 및 치료 page34~35 참조";
			case 폰히펠_린다우_증후군:				return "03. 예방 및 치료 page36~37 참조";
			case 제1형_다발성_내분비선종증:			return "03. 예방 및 치료 page38~39 참조";
			case 제2형_다발성_내분비선종증:			return "03. 예방 및 치료 page40~42 참조";
			case PTEN_과오종_증후군:				return "03. 예방 및 치료 page43~45 참조";
			case 망막모세포종:					return "03. 예방 및 치료 page46~47 참조";
			case 유전성_부신경절종_갈색세포종:		return "03. 예방 및 치료 page48~49 참조";
			case 결절성_경화증:					return "03. 예방 및 치료 page50~51 참조";
			case WT1_연관_윌름스_종양:				return "03. 예방 및 치료 page52~53 참조";
			case 제2형_신경섬유종증:				return "03. 예방 및 치료 page54~55 참조";
			default:							return "";
		}
		return null;
	}
	...
	public String clinicalMeaning(Gene gene) {
		if(gene instanceof GeneN090) switch((GeneN090)gene) {
			...
			case PALB2:		return "PALB2의 유전적 이상이 있을 경우 Pooled Odds Ratio 계산에서 약 21.4배(95% 신뢰구간, 10.10-45.32) 유전성 유방암의 위험이 증가하는 것으로 분석되어, ATM, CHEK2, BRIP1, PALB2, NBS1, RAD50 중 가장 유의미한 것으로 확인됨. 결론 PALB2 유전자 돌연변이에 의한 인종간 효과는 거의 유사하고(Heterogeneity P값 =0.947), 유전성 유방암의 위험을 약 20배 이상 증가시키는 것으로 확인되어 중요한 임상적 마커인 것으로 사료됨.";
