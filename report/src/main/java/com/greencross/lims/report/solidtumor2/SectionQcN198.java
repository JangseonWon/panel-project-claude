...
public class SectionQcN198 implements SectionQc {
    ...
    public SectionQcN198(Painter<SolidTumorTemplate, SolidTumorDto> newPage) {
    ...
    private PDPageContentStreamPageAccessible summary(PDPageContentStreamPageAccessible stream, SolidTumorTemplate template, SolidTumorDto dto) throws IOException {
        ...
        stream.paragraph(406.5f, y, 253, CENTER, MIDDLE, new TextBlock(resource.styleHeader(), "MSAF (Maximum Somatic Allele Frequency)"));
        y -= 21;
        var result = dto.qc();
        ...
        y = Math.min(y, stream.paragraph(406.5f, y, 253, CENTER, WhiteSpace.BREAK_ALL, new TextBlock(resource.styleValueTable(), fmtNumber.format(dto.qc().msaf()) + "%")));
    ...
    public PDPageContentStreamPageAccessible qc(PDPageContentStreamPageAccessible stream, SolidTumorTemplate template, SolidTumorDto dto) throws IOException {
        ...
        stream.paragraph(310, y+4, 200, new TextBlock(styleHeader, "CNV"));
        stream.setNonStrokingColor(resource.colorGray())
                .addRect(60, y, 120, -30)
                .addRect(240, y, 60, -30)
                .addRect(300, y, 120, -30)
                .addRect(480, y, 53, -30).fill()
                .line(60, y, 533, y).stroke();
        ...
        stream.paragraph(70, y+4, 100, new TextBlock(resource.styleText(), "Median Exon Coverage(x)"));
        stream.paragraph(230, y+4, 40, RIGHT, new TextBlock(resource.styleText(), fmtNumber.format(dto.qc().medianExonCoverage())));
        stream.paragraph(250, y+4, 40, new TextBlock(resource.styleText(), ">=1300"));
        stream.paragraph(310, y+4, 100, new TextBlock(resource.styleText(), "MAD"));
        stream.paragraph(470, y+4, 40, RIGHT, new TextBlock(resource.styleText(), fmtPrecision2.format(dto.qc().mad())));
        stream.paragraph(486, y+4, 40, new TextBlock(resource.styleText(), "<=0.059"));


        y-= 15;
        stream.paragraph(70, y+4, 100, new TextBlock(resource.styleText(), "Percent (PCT) Exon 1000X"));
        stream.paragraph(230, y+4, 40, RIGHT, new TextBlock(resource.styleText(), fmtNumber.format(dto.qc().pctExonOver1000X())));
        stream.paragraph(250, y+4, 40, new TextBlock(resource.styleText(), ">=80"));
        stream.paragraph(310, y+4, 100, new TextBlock(resource.styleText(), "Median bin count"));
        stream.paragraph(470, y+4, 40, RIGHT, new TextBlock(resource.styleText(), fmtPrecision.format(dto.qc().mbc())));
        stream.paragraph(486, y+4, 40, new TextBlock(resource.styleText(), ">=6.0"));
        stream.setLineWidth(1).setStrokingColor(resource.colorSecondary())
                .line(60, y, 533, y)
                .line(300, y, 300, y+45).stroke();
        stream.cursorY(y);
        stream.restoreGraphicsState();
        return stream;
    }
