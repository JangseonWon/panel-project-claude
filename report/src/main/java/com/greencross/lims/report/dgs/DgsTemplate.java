...
public interface DgsTemplate extends AbstractReportTemplate<DgsResource>, Revisionable {
	TestInfo testInfo();
	default String resultToString(DgsDto.Result result) {
		switch(result) {
	...
	default Color resultToColor(DgsDto.Result result) {
		switch(result) {
	...
	default String lblRecommendation() {
	...
	String templateEmptyIncidentalFinding();
	default String lblMethods() {
		return "METHODS";
	}
	default String lblAnalysisStatistic() { return "ANALYSIS STATISTICS"; }
