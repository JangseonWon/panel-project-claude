...
public class SectionSummaryAll implements Painter<BloodCancerTemplate, BloodCancerDto> {
    ...
    public PDPageContentStreamPageAccessible paint(PDPageContentStreamPageAccessible stream, BloodCancerTemplate template, BloodCancerDto dto) throws IOException {
        ...
        stream.setNonStrokingColor(colorPrimary).addRect(60, y-6,473, 20).fill();
        stream.paragraph(233.5f, y, 120, CENTER, new TextBlock(styleHeader2, ((AllTemplate) template).lblPanelSResult()));
        stream.paragraph(445.5f, y, 150, CENTER, new TextBlock(styleHeader2, ((AllTemplate) template).lblPharmacogeneResult()));
        y -= 25;

        stream.addRect(60, y,473, 20).fill()
                .setLineWidth(0.25f).setStrokingColor(colorSecondary)
                .line(60, y+18.1f, 533, y+18.1f)
                .line(60, y+38.1f, 533, y+38.1f).stroke()
                .setNonStrokingColor(colorPrimary);
        stream.paragraph(150.5f, y+10, 83, CENTER, MIDDLE, new TextBlock(styleHeader, template.lblTier(BloodCancerDto.Tier.Tier1)));
        stream.paragraph(233.5f, y+10, 83, CENTER, MIDDLE, new TextBlock(styleHeader, template.lblTier(BloodCancerDto.Tier.Tier2)));
        stream.paragraph(316.5f, y+10, 83, CENTER, MIDDLE, new TextBlock(styleHeader, template.lblTier(BloodCancerDto.Tier.Tier3)));
        stream.paragraph(401.75f, y+10, 87.5f, CENTER, MIDDLE, new TextBlock(styleHeader, "NUDT15 Phenotype"));
        y = stream.paragraph(489.25f, y+10, 87.5f, CENTER, MIDDLE, new TextBlock(styleHeader, "TPMT Phenotype"));
        ...
        stream.paragraph(150.5f, y, 83, CENTER, MIDDLE, new TextBlock(styleValueTable, fmtNumber.format(t1.variants()!=null?t1.variants().length:0)));
        stream.paragraph(233.5f, y, 83, CENTER, MIDDLE, new TextBlock(styleValueTable, fmtNumber.format(t2.variants()!=null?t2.variants().length:0)));
        stream.paragraph(316.5f, y, 83, CENTER, MIDDLE, new TextBlock(styleValueTable, fmtNumber.format(t3.variants()!=null?t3.variants().length:0)));
        String pheno1 = Arrays.stream(dto.drugPhenotypes()).filter(g->"NUDT15".equalsIgnoreCase(g.gene())).map(BloodCancerDto.DrugPhenotype::phenotype).findFirst().orElse("-");
        String pheno2 = Arrays.stream(dto.drugPhenotypes()).filter(g->"TPMT".equalsIgnoreCase(g.gene())).map(BloodCancerDto.DrugPhenotype::phenotype).findFirst().orElse("-");
        stream.paragraph(401.75f, y-10, 87.5f, CENTER, MIDDLE, new TextBlock(styleValueTable, pheno1));
        stream.paragraph(489.25f, y-10, 87.5f, CENTER, MIDDLE, new TextBlock(styleValueTable, pheno2));
        ...
        float y1 = stream.paragraph(150.5f, y, 83, CENTER, MIDDLE, new TextBlock(styleValueTable, g1.trim().isEmpty()?"-":g1));
        float y2 = stream.paragraph(233.5f, y, 83, CENTER, MIDDLE, new TextBlock(styleValueTable, g2.trim().isEmpty()?"-":g2));
        float y3 = stream.paragraph(316.5f, y, 83, CENTER, MIDDLE, new TextBlock(styleValueTable, g3.trim().isEmpty()?"-":g3));
