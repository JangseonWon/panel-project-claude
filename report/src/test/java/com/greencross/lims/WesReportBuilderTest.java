...
import com.greencross.lims.report.wes.WesReportBuilder;
...
@Testcontainers
class WesReportBuilderTest {
    @InjectMocks
    private WesReportBuilder reportBuilder;
    @Mock
    private Request request;
    private Des interpretation;
    ...
    private void setPositiveResult(TestInfo testInfo) {
        interpretation = new Des().result("INCONCLUSIVE")
                .resultText("MUTYH, BRIP1 유전자에서 VUS가 발견되었습니다.")
                .reasonForReferral("Multiple Myeloma")
                .variants(new Des.Variant[]{
                        new Des.Variant().gene("MUTYH").hgvsc("c.934-2A>G").hgvsp("p.(?)").zygosity("Het").disease("FAP2").inheritance("AR").clazz("VUS"),
                        new Des.Variant().gene("BRIP1").hgvsc("c.94-4A>G").hgvsp("p.(?)").zygosity("Het").disease("MIM:114480").inheritance("AD").clazz("VUS")
                }).abbreviation("Het= Heterozygote; AR= Autosomal recessive; VUS= Variant of uncertain significance; AD= Autosomal dominant; ")
                .interpretation("Adenomas, multiple colorectal의 원인 유전자인 MUTYH에서 934번째 염기서열로부터 upstream 방향으로 2번째 염기인 A가 G로 치환되는 VUS variant인 c.934-2A>G 변이가 발견되었습니다. MUTYH 유전자의 c.934-2A>G 변이는 gnomAD에서 보고된 전체 인구집단의 minor allele frequency는 0.11%이며, 한국인 인구집단(KRGDB)에서는 1.5%인 변이로 Clinvar에서 Submitter마다 서로 다르게 분류한 바 있습니다[Likely benign(3),Likely_pathogenic(2),Pathogenic(1),Uncertain_significance(5)].\n" +
                        "또한, Breast cancer, early-onset, susceptibility to의 원인 유전자인 BRIP1에서 94번째 염기서열로부터 upstream 방향으로 4번째 염기인 A가 G로 치환되는 VUS variant인 c.94-4A>G 변이가 발견되었습니다. BRIP1 유전자의 c.94-4A>G 변이는 gnomAD에서 보고된 전체 인구집단의 minor allele frequency는 0.0020%이며, 한국인 인구집단(KRGDB)에서는 0.045%인 변이로 Clinvar에서 Submitter마다 서로 다르게 분류한 바 있습니다[Likely benign(1),Uncertain_significance(1)].")
                .consentIncidentalFindings(true)
                .incidentalFindings(new Des().interpretation("ACMG에서 권고한 59개 유전자에서 (Likely) Pathogenic Variant는 발견되지 않았습니다."))
                .meanDepth("365.88").coverage("100.0").reporter("설창안").reviewer("이청화");
    }
    private void setNegativeResult(TestInfo testInfo) {
        interpretation = new Des()
                .result("NEGATIVE")
                ...
                .meanDepth("385.81").coverage("100").reporter("설창안").reviewer("이청화");
    }
}
