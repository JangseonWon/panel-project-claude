...
public interface GroupableJpa<Entity> extends SearchableJpa<Entity> {
	default Stream<GroupByResult> groupBy(Query_ query, String column) {
		...
		query.getFilters().stream().map(f -> map(cb, c, f)).filter(Objects::nonNull).reduce(cb::and).ifPresent(q::where);
