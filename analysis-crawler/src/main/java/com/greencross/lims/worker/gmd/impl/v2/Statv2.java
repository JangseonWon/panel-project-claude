...
public class Statv2 implements Worker {
	/*
	Sample Name	GC(%)	Reads	Dup(%)	OnTarget(%)	Depth(X)	0X(%)	10X>=(%)	20X>=(%)	30X>=(%)	Uniformity0.2X	Uniformity0.5X	Insert Size	SNV	INDEL	Ts/Tv	Base_Qual	Mapping_Qual	VerifybamID_FREEMIX	BI_pipeline
21GMD031_20210401-171-5028_PID-1_M	46	103562721	20.17	45.01	258.8	99.8	99.3	98.3	96.4	87.55	51.15	277.7	14058	1472	2.86	29.25	57.12	0.0028	GRM_v2.0.0
21GMD031_20210401-171-5042_DES-2_M	47	91421570	21.103	48.8	253.08	99.8	99.3	98.4	96.5	87.84	51.42	234.5	13963	1456	2.88	29.43	57.09	0.00219	GRM_v2.0.0
	 */
	private static final Pattern FILE_NAME_PATTERN = Pattern.compile("^(\\d{2}([A-Z]+)\\d{1,4}).stat_table.txt$");
	...
	public Analysis analysis(String fileName, @Nullable Request request) {
		...
		int row = Integer.parseInt(m.group(7));
		UUID sheet = SHEET_RD;
		if(request!=null) {
			...
			return new Analysis(new Analysis.AnalysisPK(sheet, batch, row, m.group(3))).panel(panel).serial(fileName);
		}
	}
	@Override
