package com.greencross.lims.worker.dgs.impl.v2;
...
public class LimsStatv2 implements Worker {
	...
	private static final Pattern SERIAL_PATTERN = Pattern.compile("^((\\d{2}DGS\\d{1,4})(-RE)*[_-]([A-Za-z0-9]+)-(\\d{1,4})[_-](\\d{8}-\\d{3}-\\d{4}))");
	...
	public Analysis analysis(String fileName, @Nullable Request request) {
		...
		} else return new Analysis(new Analysis.AnalysisPK(sheet, batch, row, m.group(6))).panel(panel).serial(fileName);
	}
	@Override
