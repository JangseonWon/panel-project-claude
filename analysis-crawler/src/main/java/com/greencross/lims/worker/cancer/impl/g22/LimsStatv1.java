...
@Service("G22 CANCER-LIMS Stat ver.1")
public class LimsStatv1 implements Worker {
	 ...
	 */
	private static final Pattern FILE_NAME_PATTERN = Pattern.compile("^(\\d{2}G22\\d{5}-*Cancer\\d{1,4}).LIMS_stat.txt$");
	// 21GMD030_20210330-171-5130_SD-4_M
	private static final Pattern SERIAL_PATTERN = Pattern.compile("^(((\\d{2}G22\\d{5}-*Cancer\\d{1,4})[-_]((\\d{8}-\\d{3}-\\d{4})|(?i)(positive))([^.]*?))[-_](((.+?)-(\\d{1,4}))|(\\d{2}(\\D+)\\d{1,4}?))?[-_](\\D{1,4}))$");
	...
	@Override
	public String name() {
		return "G22 CANCER-LIMS Stat ver.1";
	}
