...
@Service("G22 Cancer-SNV-Multimapping ver.1")
public class SnvMultimappingv1 extends SnvWorkerImpl {
	private static final Pattern FILE_NAME_PATTERN = Pattern.compile("^(((\\d{2}G22\\d{5}-*Cancer\\d{1,4})[-_]((\\d{8}-\\d{3}-\\d{4})|(?i)(positive))([^.]*?))[-_](((.+?)-(\\d{1,4}))|(\\d{2}(\\D+)\\d{1,4}?))?[-_](\\D{1,4}))\\.multimapping\\.annotation.txt$");
	...
	public SnvMultimappingv1(ElasticsearchRestTemplate em, SnvCandidateDAO candidateDao, SnvConsensualClassDao consensualDao, RequestDAO requestDAO, ObjectMapper om) {
	...
	@Override
	public String name() {
		return "G22 Cancer-SNV-Multimapping ver.1";
	}
	@Override
