...
@Service("GMD-LIMS Stat ver.3")
public class LimsStatv3 implements Worker {
	private static final Pattern FILE_NAME_PATTERN = Pattern.compile("^(\\d{2}[A-Z]+\\d{1,4}).LIMS_stat.txt$");
	// 21GMD030_20210330-171-5130_SD-4_M
	private static final Pattern SERIAL_PATTERN = Pattern.compile("^((\\d{2}([A-Z]+)\\d{1,4})(-RE)*[_-]([A-Za-z0-9]+)-(\\d{1,4})[_-](\\d{8}-\\d{3}-\\d{4}))");
	...
	public LimsStatv3(RequestDAO requestDAO) {
	...
	public Analysis analysis(String fileName, @Nullable Request request) {
		...
		if(request!=null) {
			...
			return new Analysis(new Analysis.AnalysisPK(sheet, batch, row, m.group(7))).panel(panel).serial(fileName);
		}
	}
	@Override
