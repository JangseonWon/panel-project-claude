...
@Service("Cancer-SnvCount ver.2")
public class SnvCountv2 extends SnvCountWorker {
	private static final Pattern FILE_NAME_PATTERN = Pattern.compile("^(\\d{2}(?:OS-)*Cancer\\d{1,4}).variant_count_LIMS.txt$");
	// 21Cancer024_GSC-06_20210324-171-5133
	private static final Pattern SERIAL_PATTERN = Pattern.compile("^(((\\d{2}(?:OS-)*Cancer\\d{1,4})[-_]((\\d{8}-\\d{3}-\\d{4})|(?i)(positive))([^.]*?))[-_](((.+?)-(\\d{1,4}))|(\\d{2}(\\D+)\\d{1,4}?))?[-_](\\D{1,4}))\\.annotation\\.txt$");
	...
	public SnvCountv2(AnalysisDAO dao) {
		super(dao);
		this.dao = dao;
	...
	protected Long sample(String fileName) {
		...
		long val =  Long.parseLong(m.group(4).replace("-", ""));
