...
@Service("GMD-SNV-Candidates ver.3")
public class SnvCandidatesv3 extends SnvWorkerImpl {
	private static final Pattern FILE_NAME_PATTERN = Pattern.compile("^((\\d{2}([A-Z]+)\\d{1,4})(-RE)*[_-]([A-Za-z0-9]+)-(\\d{1,4})[_-](\\d{8}-\\d{3}-\\d{4})(.*)).annotation.filter_Candidates.txt$");
	...
	public SnvCandidatesv3(ElasticsearchRestTemplate em, SnvCandidateDAO candidateDao, SnvConsensualClassDao consensualDao, RequestDAO requestDAO, ObjectMapper om) {
	...
	@Override
	public String name() {
		return "GMD-SNV-Candidates ver.3";
	}
	@Override
