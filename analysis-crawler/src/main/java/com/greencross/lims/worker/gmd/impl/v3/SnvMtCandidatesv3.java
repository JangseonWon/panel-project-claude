package com.greencross.lims.worker.gmd.impl.v3;
...
@Service("GMD-SNV-MT-Candidates ver.3")
public class SnvMtCandidatesv3 extends SnvWorkerImpl {
	private static final Pattern FILE_NAME_PATTERN = Pattern.compile("^((\\d{2}([A-Z]+)\\d{1,4})(-RE)*[_-]([A-Za-z0-9]+)-(\\d{1,4})[_-](\\d{8}-\\d{3}-\\d{4})(.*))_MT.annotation_filter.txt$");
	...
	public static final Process PROCESSOR = new Process() {
		...
		public Long sample(String fileName) {
			...
			if(m.group(7) == null || m.group(7).isBlank()) return null;
			return Long.parseLong(m.group(7).replace("-", ""));
		}
		@Override
		...
		public Analysis map(String fileName, @Nullable Request request) {
			...
			if(request!=null) {
				...
				return new Analysis(new Analysis.AnalysisPK(sheet, batch, row, m.group(7))).panel(panel).serial(serial);
			}
		}

		@Override
		public List<String> tags(Map<String, String> values) {
			return List.of("candidate" ,"mt");
		}
	};
	private final List<String> services = new LinkedList<>();
	public SnvMtCandidatesv3(ElasticsearchRestTemplate em, SnvCandidateDAO candidateDao, SnvConsensualClassDao consensualDao, RequestDAO requestDAO, ObjectMapper om) {
	...
	@Override
	public String name() {
		return "GMD-SNV-MT-Candidates ver.3";
	}
	@Override
