...
@Service("CANCER-LIMS Stat ver.3")
public class LimsStatv3 implements Worker {
	private static final Pattern FILE_NAME_PATTERN = Pattern.compile("^(\\d{2}(?:.{8})*[-A-Za-z]+\\d{1,4}).LIMS_stat.txt$");
	private static final Pattern SERIAL_PATTERN = Pattern.compile("^((\\d{2}(?:OS-)*Cancer\\d{1,4})[_-]([A-Za-z0-9]+)-(\\d{1,4})[_-](\\d{8}-\\d{3}-\\d{4}))");
	...
	public Long sample(String fileName) {
		...
		long val =  Long.parseLong(m.group(5).replace("-", ""));
