...
@Service("Cancer-SNV-Candidates ver.3")
public class SnvCandidatesv3 extends SnvWorkerImpl {
	private static final Pattern FILE_NAME_PATTERN = Pattern.compile("^((\\d{2}(?:OS-)*Cancer\\d{1,4})[_-]([A-Za-z0-9]+)-(\\d{1,4})[_-](\\d{8}-\\d{3}-\\d{4}))\\.annotation\\.filter_Candidates\\.txt$");
	...
	@Override
	public String name() {
		return "Cancer-SNV-Candidates ver.3";
	}
	@Override
