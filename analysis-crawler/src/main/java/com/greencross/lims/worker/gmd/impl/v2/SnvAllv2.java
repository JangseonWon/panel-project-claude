...
@Service("GMD-SNV ver.2")
public class SnvAllv2 extends SnvWorkerImpl {
	private static final Pattern FILE_NAME_PATTERN = Pattern.compile("^((\\d{2}([A-Z]+)\\d{1,4})(-RE)*[-_]((\\d{8}-\\d{3}-\\d{4})|(?i)(.+))[-_](.+)[-_](\\d{1,4})(.*))_[MI].annotation.txt$");
	...
	public static final SnvWorkerImpl.Process PROCESSOR = new SnvWorkerImpl.Process() {
		@Override
	...
	public SnvAllv2(ElasticsearchRestTemplate em, SnvCandidateDAO candidateDao, SnvConsensualClassDao consensualDao, RequestDAO requestDAO, ObjectMapper om) {
		...
		// for(com.greencross.lims.test.wes.TestInfo t: com.greencross.lims.test.wes.TestInfo.TESTS) services.add(t.code());
	}
