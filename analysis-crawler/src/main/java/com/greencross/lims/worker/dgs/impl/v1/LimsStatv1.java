...
@Service("DGS-LIMS Stat ver.1.1")
public class LimsStatv1 implements Worker {
	private static final Pattern FILE_NAME_PATTERN = Pattern.compile("^(\\d{2}DGS\\d{1,4}).LIMS_stat.txt$");
	private static final Pattern SERIAL_PATTERN = Pattern.compile("^(\\d{2}DGS\\d{1,4})(-RE)*[-_]((\\d{8}-\\d{3}-\\d{4})|(?i)(.+))[-_](.+)[-_](\\d{1,4})(.*)$");
	...
	public boolean chk(File file) {
		if(!"v".equalsIgnoreCase(file.getParentFile().getName())) return false;
	...
	public Analysis analysis(String fileName, @Nullable Request request) {
		...
		if(request!=null && "wgs".equalsIgnoreCase(panel)) {
		...
		} else return new Analysis(new Analysis.AnalysisPK(sheet, batch, row, m.group(3))).panel(panel).serial(fileName);
	}
	@Override
