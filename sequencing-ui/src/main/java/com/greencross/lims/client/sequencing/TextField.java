...
public abstract class TextField<V> extends HTMLElementBuilder<HTMLLabelElement, TextField<V>> implements HasValueChangeHandlers<V>, HasClickHandlers {
    ...
    public static final class TextFieldBuilder<V> {
        private InputBuilder<HTMLInputElement> input;
        private HtmlContentBuilder<?> proxy;
        ...
        public TextField<V> filled() {
            ...
            if(proxy!=null) input.element().parentElement.insertBefore(proxy.element(), input.element().nextElementSibling);
    ...
    public static TextFieldBuilder<Sheet.Link> fileBox() {
        InputBuilder<HTMLInputElement> input = Elements.input(InputType.text).css("mdc-text-field__input", "file").style("cursor: pointer; caret-color: transparent;");
       // input.element().value="LINK";
        input.on(EventType.click, evt->{
            evt.stopPropagation();
            evt.preventDefault();
        ...
        HtmlContentBuilder<HTMLAnchorElement> upload = a("#").style("position: absolute; right: 0.5em; top: 50%; bottom: 50%; transform: translateY(-0.7em);").add("Upload");
        upload.on(EventType.click, evt->{
            evt.stopPropagation();
            evt.preventDefault();
            evt.stopImmediatePropagation();
        });
        return new TextFieldBuilder<Sheet.Link>().input(input).proxy(upload)
                .setter(v->{
                    if(v==null) return null;
                    return v.label();
                }).getter(()-> new Sheet.Link().label(input.element().value));
    }
    ...
    private static final class MDCTextField {
        private String value;
        private boolean disabled;
        private boolean valid;
    }
    @JsType(isNative = true, namespace= JsPackage.GLOBAL, name="Object")
    private static final class MDCTextFieldFoundation {
    ...
    @JsType(isNative = true, namespace= JsPackage.GLOBAL, name="Object")
    private static final class MDCTextFieldHelperText {

    }
    @JsType(isNative = true, namespace= JsPackage.GLOBAL, name="Object")
    private static final class MDCTextFieldHelperTextFoundation {
