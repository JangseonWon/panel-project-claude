...
@Repository
public class AnalysisDnaDAO extends AbstractJpaDAO<AnalysisDna> implements GroupableJpa<AnalysisDna> {
	@Override
	public Predicate map(CriteriaBuilder cb, Root<AnalysisDna> c, Query.Filter filter) {
		return map(cb, c, filter.key(), filter.value());
	}
	private Predicate map(CriteriaBuilder cb, Root<AnalysisDna> c, String key, String value) {
	...
	public Stream<AnalysisDna> list(int batch, Query query) {
		String sort = query.sortBy();
		boolean isAsc = query.asc();
		CriteriaBuilder cb = builder();
		CriteriaQuery<AnalysisDna> q = cb.createQuery(entityClass());
		Root<AnalysisDna> c = q.from(entityClass());
		Predicate where = cb.equal(c.get("batch"), em().getReference(BatchDna.class, new BatchDna.BatchDnaPK(batch)));
	...
	public Optional<Integer> findLastRow(BatchDna parent) {
		CriteriaBuilder cb = builder();
		CriteriaQuery<Integer> q = cb.createQuery(Integer.class);
		Root<AnalysisDna> c = q.from(AnalysisDna.class);
