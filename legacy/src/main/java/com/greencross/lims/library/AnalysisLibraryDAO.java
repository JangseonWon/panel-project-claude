...
@Repository
public class AnalysisLibraryDAO extends AbstractJpaDAO<AnalysisLibrary> implements GroupableJpa<AnalysisLibrary> {
	@Override
	public Predicate map(CriteriaBuilder cb, Root<AnalysisLibrary> c, Query.Filter filter) {
		return map(cb, c, filter.key(), filter.value());
	}
	private Predicate map(CriteriaBuilder cb, Root<AnalysisLibrary> c, String key, String value) {
	...
	public Stream<AnalysisLibrary> list(int batch, Query query) {
		String sort = query.sortBy();
		boolean isAsc = query.asc();
		CriteriaBuilder cb = builder();
		CriteriaQuery<AnalysisLibrary> q = cb.createQuery(entityClass());
		Root<AnalysisLibrary> c = q.from(entityClass());
		Predicate where = cb.equal(c.get("batch"), em().getReference(BatchLibrary.class, new BatchLibrary.BatchLibraryPK(batch)));
	...
	public Optional<Integer> findLastRow(BatchLibrary parent) {
		CriteriaBuilder cb = builder();
		CriteriaQuery<Integer> q = cb.createQuery(Integer.class);
		Root<AnalysisLibrary> c = q.from(AnalysisLibrary.class);
