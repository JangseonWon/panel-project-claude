...
@Repository
public class BatchLibraryDAO extends AbstractJpaDAO<BatchLibrary> implements GroupableJpa<BatchLibrary> {
	@Override
	public Predicate map(CriteriaBuilder cb, Root<BatchLibrary> c, Query.Filter filter) {
		return map(cb, c, filter.key(), filter.value());
	}
	private Predicate map(CriteriaBuilder cb, Root<BatchLibrary> c, String key, String value) {
	...
	public Optional<Integer> findLastBatch() {
		...
		Root<BatchLibrary> c = q.from(BatchLibrary.class);
		Expression<Integer> maxExp = cb.max(c.get("batch"));
		q.select(maxExp).where(cb.equal(c.get("sheet"), em().getReference(BatchTemplate.class, BatchLibrary._ID)));
