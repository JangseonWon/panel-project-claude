...
public class LibraryAnalysisService {
	private final AnalysisLibraryDAO dao;
	public LibraryAnalysisService(AnalysisLibraryDAO dao) {
		this.dao = dao;
	}
	...
	public Optional<Analysis> get(int batch, int row) {
		return dao.find(AnalysisLibrary.AnalysisLibraryPK.builder().batch(batch).row(row).build()).map(AnalysisToDTO::map);
	}
	@Transactional(readOnly = true)
	public int findLastRow(int batch) {
		BatchLibrary parent = dao.em().getReference(BatchLibrary.class, BatchLibrary.BatchLibraryPK.builder().batch(batch).build());
		return dao.findLastRow(parent).orElse(0);
	}
	@Transactional
	public AnalysisLibrary create(int batch, int row, RequestReference target) {
		AnalysisLibrary entity = new AnalysisLibrary(AnalysisLibrary.AnalysisLibraryPK.builder().batch(batch).row(row).build());
		entity.batch(dao.em().getReference(BatchLibrary.class, BatchLibrary.BatchLibraryPK.builder().batch(batch).build())).row(entity.pk().row());
		if(target!=null) {
	...
	public Analysis update(int batch, int row, Map<String, String> values) {
		return dao.find(AnalysisLibrary.AnalysisLibraryPK.builder().batch(batch).row(row).build())
				  .map(entity -> {
				  ...
				  .orElseThrow(() -> new RuntimeException("Can't find AnalysisLibrary:" + batch + ", " + row));
	}
	@Transactional
	public void delete(int batch, int row) {
		dao.remove(dao.em().getReference(AnalysisLibrary.class, AnalysisLibrary.AnalysisLibraryPK.builder().batch(batch).row(row).build()));
	}
	@Transactional
