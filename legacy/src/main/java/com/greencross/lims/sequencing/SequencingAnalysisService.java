...
@Service
public class SequencingAnalysisService {
	private final AnalysisSequencingDAO dao;
	public SequencingAnalysisService(AnalysisSequencingDAO dao) {
		this.dao = dao;
	}
	...
	public Optional<Analysis> get(int batch, int row) {
		return dao.find(AnalysisSequencing.AnalysisSequencingPK.builder().batch(batch).row(row).build()).map(AnalysisToDTO::map);
	}
	@Transactional(readOnly = true)
	public int findLastRow(int batch) {
		BatchSequencing parent = dao.em().getReference(BatchSequencing.class, BatchSequencing.BatchSequencingPK.builder().batch(batch).build());
		return dao.findLastRow(parent).orElse(0);
	}
	@Transactional
	public AnalysisSequencing create(int batch, int row, RequestReference target) {
		AnalysisSequencing entity = new AnalysisSequencing(AnalysisSequencing.AnalysisSequencingPK.builder().batch(batch).row(row).build());
		entity.batch(dao.em().getReference(BatchSequencing.class, BatchSequencing.BatchSequencingPK.builder().batch(batch).build())).row(entity.pk().row());
		if(target!=null) {
	...
	public Analysis update(int batch, int row, Map<String, String> values) {
		return dao.find(AnalysisSequencing.AnalysisSequencingPK.builder().batch(batch).row(row).build())
				.map(entity -> {
				...
				.orElseThrow(() -> new RuntimeException("Can't find AnalysisSequencing:" + batch + ", " + row));
	}
	@Transactional
	public void delete(int batch, int row) {
		dao.remove(dao.em().getReference(AnalysisSequencing.class, AnalysisSequencing.AnalysisSequencingPK.builder().batch(batch).row(row).build()));
	}
	@Transactional
