...
@Repository
public class BatchSequencingDAO extends AbstractJpaDAO<BatchSequencing> implements GroupableJpa<BatchSequencing> {
	@Override
	public Predicate map(CriteriaBuilder cb, Root<BatchSequencing> c, Query.Filter filter) {
		return map(cb, c, filter.key(), filter.value());
	}
	private Predicate map(CriteriaBuilder cb, Root<BatchSequencing> c, String key, String value) {
	...
	public Optional<Integer> findLastBatch() {
		...
		Root<BatchSequencing> c = q.from(BatchSequencing.class);
		Expression<Integer> maxExp = cb.max(c.get("batch"));
		q.select(maxExp).where(cb.equal(c.get("sheet"), em().getReference(BatchTemplate.class, BatchSequencing._ID)));
