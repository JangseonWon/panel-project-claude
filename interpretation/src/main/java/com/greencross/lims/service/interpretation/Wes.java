...
@Component
public class Wes implements Interpretable {
	private final InterpretationDAO dao;
	private final SnvDAO snvRepo;
	private final ObjectMapper om;
	public Wes(InterpretationDAO dao, SnvDAO snvRepo, ObjectMapper om) {
		this.dao = dao;
	...
	public Mono<Object> interpret(long sample, String service, InterpretationParam param) {
		...
		com.greencross.lims.dto.interpretation.Des prev = om.convertValue(param.json(), com.greencross.lims.dto.interpretation.Des.class);
		...
		if(prev.variants()==null || prev.variants().length <= 0) {
			...
			prev.interpretation(NEGATIVE_FORMAT);
		} else {
			List<Des.Variant> variants = new LinkedList<>(Arrays.asList(prev.variants()));
			...
			for(Des.Variant variant: prev.variants()) {
		...
		if(prev.incidentalFindings()==null || prev.incidentalFindings().variants()==null ||
			...
			List<Des.Variant> variants = Arrays.stream(prev.incidentalFindings().variants())
					.filter(c -> reportable(c.clazz()))
			...
			List<String> abbreviationInheritance = new ArrayList<>();
			List<String> abbreviationClass = new ArrayList<>();
			for(Des.Variant variant: prev.incidentalFindings().variants()) {
				String snv = variant.snv();
				String analysis = variant.analysis();
				String batch = analysis.trim().split(":", -1)[1];
				String gene = variant.gene();
	...
	private static String resultText(String result, List<Des.Variant> variants) {
	...
	protected String header = "DES 분석 결과, ";
	public String resultHeader(List<Des.Variant> variants) {
		StringBuilder sb = new StringBuilder(header);
		Map<String, List<Des.Variant>> vars = variants.stream().collect(Collectors.groupingBy(Des.Variant::clazz));
		List<Des.Variant> Ps = vars.getOrDefault("P", new ArrayList<>());
		Ps.addAll(vars.getOrDefault("PV", new ArrayList<>()));
		List<Des.Variant> LPs = vars.getOrDefault("LP", new ArrayList<>());
		LPs.addAll(vars.getOrDefault("LPV", new ArrayList<>()));
		List<Des.Variant> VUSs = vars.getOrDefault("VUS", List.of());
	...
	private String addLineToResultHeader(List<Des.Variant> variants, String clazz){
	...
	private static final String NEGATIVE_FORMAT = "검사 결과 임상적으로 의미 있는 변이가 발견되지 않았습니다.\n" +
			"본 검사는 검사에 포함된 유전자들의 모든 coding exon과 인접 intron 영역을 분석하며, splicing에 영향을 미치지 않을 " +
			"것으로 예측되는 synonymous variant 및 intronic variant는 보고하지 않습니다. 또한 exon deletion/duplication, " +
			"genomic rearrangement를 포함한 copy number variant는 검출하는데 한계가 있습니다. 검사의 기술적 한계에 대한 자세한 " +
			"내용은 아래 검사의 한계를 참조하십시오.";
	@Override
	public Object negative(long sample, String service) {
		var interpretation = new com.greencross.lims.dto.interpretation.Des();
		interpretation.result("NEGATIVE").resultText("의뢰 사유와 관련된 변이는 발견되지 않았습니다.");
		interpretation.interpretation(NEGATIVE_FORMAT);
		return interpretation;
	}
	private String abbreviationReference(Des.Variant variant) {
	...
	private String abbreviationZygosity(Patient.Sex sex, Des.Variant variant, Document values) {
	...
	private String[] abbreviationInheritance(Des.Variant variant, List<InterpretationParam.InterpretationParamDisease> param) {
	...
	private String abbreviationClass(Des.Variant variant) {
		return variant.clazz() + ", " + Phrase.classLong(variant.clazz());
	}
	private String comment(Des.Variant variant, Document values, List<InterpretationParam.InterpretationParamDisease> diseases, AtomicBoolean ms) {
	...
	private String commentHgvs(Des.Variant variant, Document values) {
		try {
	...
	private String commentPopulation(Des.Variant variant, Document values, AtomicBoolean hasPreviousPopulationComment) {
		try {
	...
	private String commentInsilico(Des.Variant variant, Document values) {
		try {
	...
	private String commentClinvarHgmd(Des.Variant variant, Document values) {
		try {
	...
	private String commentClinvar(Des.Variant variant, Document values) {
		try {
	...
	private String commentHgmd(Des.Variant variant, Document values) {
		try {
	...
	private String commentDisease(Des.Variant variant, List<InterpretationParam.InterpretationParamDisease> diseases) {
		try {
