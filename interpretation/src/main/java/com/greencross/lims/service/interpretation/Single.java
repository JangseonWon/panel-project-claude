...
@Component
public class Single implements Interpretable {
	private final InterpretationDAO dao;
	private final SnvDAO snvRepo;
	private final ObjectMapper om;
	public Single(InterpretationDAO dao, SnvDAO snvRepo, ObjectMapper om) {
		this.dao = dao;
	...
	public boolean chk(long sample, String service) {
		for(TestInfo test: TestInfo.TESTS) if(test.code().equalsIgnoreCase(service) && !test.gene().contains("BRCA")) return true;
		return false;
	}
	...
	public Mono<Object> interpret(long sample, String service, InterpretationParam param) {
		...
		if(prev.variants()==null || prev.variants().length <= 0) {
			...
			prev.interpretation(NEGATIVE_FORMAT.replace("%g", test.gene()));
		} else {
			...
			List<PanelTest.Variant> lbv = variants.stream().filter(v->"LBV".equalsIgnoreCase(v.clazz())).collect(Collectors.toList());
			List<PanelTest.Variant> bv = variants.stream().filter(v->"BV".equalsIgnoreCase(v.clazz())).collect(Collectors.toList());
			...
			for(PanelTest.Variant variant: prev.variants()) {
				...
				abbreviationReference.add(abbreviationReference(variant, values));
		...
		if(prev.addendum()==null || prev.addendum().variants()==null || prev.addendum().variants().length <= 0) prev.addendum(null);
			...
			String summary = "추가로 분석된 " + summary(test, variants);
	...
	private static final String NEGATIVE_FORMAT = "%g 유전자의 모든 exon과 인접 intron의 염기서열을 분석한 결과, 질환 관련 변이는 발견되지 않았습니다.";
	public Object negative(long sample, String service) {
		...
		interpretation.interpretation(NEGATIVE_FORMAT.replace("%g", test.gene()));
		return interpretation;
	}

	private String abbreviationReference(PanelTest.Variant variant, Document values) {
	...
	private String summary(TestInfo test, List<PanelTest.Variant> variants) {
		...
		sb.append(
				variants.stream()
						...
						.map(v->summaryGene(test, v.gene(), hit.get(v.gene())))
						.collect(Collectors.joining(" 발견되었고, "))
		).append(" 발견되었습니다.");
		return sb.toString();
	}
	private String summaryGene(TestInfo test, String gene, Set<String> classes) {
		StringBuilder sb = new StringBuilder();
		if(!test.gene().equals(gene)) sb.append("%g 유전자에서 ".replace("%g", gene));
		String acmgs = classes.stream().sorted(Comparator.comparing(Phrase::ordinalClass))
							  .map(Phrase::classFull)
