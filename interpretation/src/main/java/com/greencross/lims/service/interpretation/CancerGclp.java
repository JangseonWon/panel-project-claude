...
// GCLP, GRM v2.0 유지
@Component
public class CancerGclp implements Interpretable {
	private final InterpretationDAO dao;
	private final SnvDAO snvRepo;
	private final ObjectMapper om;
	public CancerGclp(InterpretationDAO dao, SnvDAO snvRepo, ObjectMapper om) {
		this.dao = dao;
	...
	public boolean chk(long sample, String service) {
		if(G2200101.code().equals(service)) return true;
		if(G2200201.code().equals(service)) return true;
		return false;
	}
	...
	private static final String NEGATIVE_FORMAT = "유전성 암 유전자 패널 분석 결과, 질환과 관련된 변이가 발견되지 않았습니다.\n\n" +
	...
	private static final String GNOMAD_KEY = "gnomad.exome.all";
	private static final String KRGDB_KEY = "krg_db_1100";
	...
	private static final String POLY_PRED_KEY = "polyphen2.hvar.pred";
	...
	private String commentInsilico(Document values) {
		...
		boolean sift = !"T".equalsIgnoreCase(values.getString(SIFT_PRED_KEY));
		boolean polyphen = !"B".equalsIgnoreCase(values.getString(POLY_PRED_KEY));
