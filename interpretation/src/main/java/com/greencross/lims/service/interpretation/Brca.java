...
@Component
public class Brca implements Interpretable {
	private final InterpretationDAO dao;
	private final SnvDAO snvRepo;
	private final ObjectMapper om;
	public Brca(InterpretationDAO dao, SnvDAO snvRepo, ObjectMapper om) {
		this.dao = dao;
	...
	public boolean chk(long sample, String service) {
		...
		for(TestInfo test: TestInfo.TESTS_BRCA) if(test.code().equalsIgnoreCase(service)) return true;
		for(com.gcgenome.lims.test.single.TestInfo test: com.gcgenome.lims.test.single.TestInfo.TESTS)
			if(test.code().equalsIgnoreCase(service) && test.gene().contains("BRCA")) return true;
		return false;
	}

	private <T extends HasReferralDefault & HasGene> T test(String service) {
		for(TestInfo test: TestInfo.TESTS_BRCA) if(test.code().equalsIgnoreCase(service)) return (T) test;
		for(com.gcgenome.lims.test.single.TestInfo test: com.gcgenome.lims.test.single.TestInfo.TESTS)
			if(test.code().equalsIgnoreCase(service) && test.gene().contains("BRCA")) return (T) test;
		return null;
	}
	...
	public Mono<Object> interpret(long sample, String service, InterpretationParam param) {
		...
		prev.reasonForReferral("R/O " + test(service).referralDefault());
		...
		if(prev.variants()==null || prev.variants().length <= 0) prev.result("NEGATIVE").resultText("질환과 관련된 변이는 발견되지 않았습니다.").interpretation(NEGATIVE_FORMAT.replace("%g", test(service).gene()));
		else {
	...
	public Object negative(long sample, String service) {
		HasReferralDefault test = test(service);
		String gene = test(service).gene();
		...
		interpretation.result("NEGATIVE")
					  ...
					  .interpretation(NEGATIVE_FORMAT.replace("%g", gene));
	...
	private String positive(Patient patient, String gene, List<PanelTest.Variant> variants, AtomicBoolean hasMaf) {
		...
		String comment = variants.stream()
								 .sorted(Comparator.comparing(v->Phrase.ordinalClass(v.clazz())))
