...
import com.greencross.lims.dto.interpretation.Des.Variant;
...
@Component
public class WesEnUs implements Interpretable {
	private final InterpretationDAO dao;
	private final SnvDAO snvRepo;
	private final ObjectMapper om;
	public WesEnUs(InterpretationDAO dao, SnvDAO snvRepo, ObjectMapper om) {
		this.dao = dao;
	...
	public boolean chk(long sample, String service) {
		for(com.gcgenome.lims.test.wes.TestInfo test: com.gcgenome.lims.test.wes.TestInfo.TESTS) if(test.code().equalsIgnoreCase(service)) return "ENUS".equalsIgnoreCase(test.i18n());
		for(TestInfo test: TestInfo.TESTS) if(test.code().equalsIgnoreCase(service)) return "ENUS".equalsIgnoreCase(test.i18n());
		return false;
	}

	protected final static String NEGATIVE_RESULT_TEXT = "No pathogenic variants were detected, related to the patient’s clinical phenotype.";
	private final static String NEGATIVE_INTERPRETATION = "Analysis of whole exome sequencing found no disease-related variant.\n" +
			"This test analyzes the coding exons and adjacent intron areas of the genes included in the panel " +
			"and cannot detect deep intronic mutations, exon deletions/duplications, and copy number variants " +
			"including genomic rearrangements, etc. For more information on the technical limitations of a test, " +
			"see the limits of the test below.";
	@Override
	...
	public Mono<Object> interpret(long sample, String service, InterpretationParam param) {
		...
		Des prev = om.convertValue(param.json(), Des.class);
		...
		for(Variant variant: prev.variants()) {
		...
		String interpretation = date + interpretationPerGenes.stream().collect(Collectors.joining("\n\n"));
		...
		if(prev.incidentalFindings()==null || prev.incidentalFindings().variants()==null ||
			...
			resultText = resultText(prev.incidentalFindings().variants(), patient.sex(), param.disease());
			abbreviationReference = new ArrayList<>();
			abbreviationDisease = new ArrayList<>();
			abbreviationZygosity = new ArrayList<>();
			abbreviationInheritance = new ArrayList<>();
			abbreviationClass = new ArrayList<>();
			interpretationPerGenes = new LinkedList<>();
			for (Variant variant : prev.incidentalFindings().variants()) {
				String snv = variant.snv();
				String analysis = variant.analysis();
				String batch = analysis.trim().split(":", -1)[1];
				String gene = variant.gene();
				...
				String abbrZyg = abbreviationZygosity(patient.sex(), variant, values);
				abbreviationReference.add(abbreviationReference(variant));
				for (InterpretationParam.InterpretationParamDisease info : diseases) {
					abbreviationDisease.add(abbreviationDisease(info));
					abbreviationZygosity.add(abbrZyg);
				...
				abbrReference = abbreviationReference.stream().distinct().collect(Collectors.joining("; "));
				abbrDisease = abbreviationDisease.stream().distinct().filter(Objects::nonNull).filter(s -> s.trim().length() > 0).collect(Collectors.joining("; "));
				abbr = Stream.concat(Stream.concat(abbreviationZygosity.stream(), abbreviationInheritance.stream()), abbreviationClass.stream())
						.distinct().sorted().collect(Collectors.joining("; "));
				interpretation = interpretationPerGenes.stream().collect(Collectors.joining("\n\n"));
				prev.incidentalFindings().resultText(resultText)
						.abbreviationReference(abbrReference).abbreviationDisease(abbrDisease).abbreviation(abbr)
						.interpretation(interpretation);
			}
		}
	...
	private String resultText(Variant[] variants, Patient.Sex sex, Map<String, List<InterpretationParam.InterpretationParamDisease>> param) {
		...
		return genes.stream().map(gene->{
			Variant[] target = Arrays.stream(variants).filter(v->gene.equalsIgnoreCase(v.gene())).toArray(Variant[]::new);
			return resultTextPerGene(gene, target, sex, param.get(gene));
		}).collect(Collectors.joining(";"));
	}
	private String resultTextPerGene(String gene, Variant[] variants, Patient.Sex sex, List<InterpretationParam.InterpretationParamDisease> param) {
		if(variants == null || variants.length <= 0) return "";
			...
			return "A {:zyg}{:cls} was identified in the {:gene} gene, related to the patient’s clinical phenotype."
			...
			String text = classes.stream().collect(Collectors.joining(" and ")) +
					" were  identified in the {:gene} gene, related to the patient’s clinical phenotype."
							.replace("{:gene}", gene);
	...
	private String abbreviationReference(Variant variant) {
	...
	private String abbreviationZygosity(Patient.Sex sex, Variant variant, Document values) {
	...
	private String[] abbreviationInheritance(Variant variant, List<InterpretationParam.InterpretationParamDisease> param) {
	...
	private String abbreviationClass(Variant variant) {
		return variant.clazz() + ", " + Phrase.classLong(variant.clazz());
	}
	private String interpretationPerGene(Variant variant, Patient.Sex sex, List<InterpretationParam.InterpretationParamDisease> param, Document values) {
		String gene = variant.gene();
		String hgvsc = variant.hgvsc();
		String hgvsp = variant.hgvsp();

		String header = "{:gene}, {:hgvs}\n".replace("{:gene}", gene);
	...
	private String literature(Variant variant, Document values) {
		try {
	...
	private String conclusion(Variant variant) {
	...
	private String diseaseDescription(Variant variant, List<InterpretationParam.InterpretationParamDisease> param) {
		StringBuilder sb = new StringBuilder("\r\nPathogenic ");
