...
public class BatchElement extends AbstractScenePageable<BatchElement> {
	...
	private final ButtonElement next = ButtonElement.outline().css("button").text("Library QC").before(IconElement.icon(IconElement.Type.Regular, "fa-vials"));
	...
	private void next() {
		...
		if(!DomGlobal.confirm("선택한 " + selection.length + "개 배치를 Library QC로 복사합니다.")) return;
		...
		BatchApi.checkout(refs, result->update());
	}
	@Override
