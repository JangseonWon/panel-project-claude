...
public class BallondorCollapseElement extends HTMLElementBuilder<HTMLDivElement, BallondorCollapseElement> implements CollapseElement<HTMLDivElement> {
	public static BallondorCollapseElement build(String id, long sample, String service) {
		return new BallondorCollapseElement(div(), id, sample, service);
	}
	...
	private BallondorCollapseElement(HTMLContainerBuilder<HTMLDivElement> e, String id, long sample, String service) {
	...
	public void update() {
		...
		InterpretationApi.interpretation(sample, service).then(reports->{
			this.update((BallondorDto)reports);
			return null;
		});
	}

	private void update(BallondorDto dto) {
		meta.element().innerHTML = "";
		if(dto!=null && dto.summary()!=null) {
			meta.add(summary.value(dto.summary())).style("flex-basis: 100%;");
	...
	@Override
	public BallondorCollapseElement that() {
		return this;
	}
