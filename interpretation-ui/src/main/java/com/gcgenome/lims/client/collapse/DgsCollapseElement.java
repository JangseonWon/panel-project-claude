...
import com.gcgenome.lims.dto.interpretation.Dgs;
...
public class DgsCollapseElement extends HTMLElementBuilder<HTMLDivElement, DgsCollapseElement> implements CollapseElement<HTMLDivElement> {
	public static DgsCollapseElement build(String id, long sample, String service) {
		return new DgsCollapseElement(div(), id, sample, service);
	}
	...
	private DgsCollapseElement(HTMLContainerBuilder<HTMLDivElement> e, String id, long sample, String service) {
	...
	private void layout() {
		_this.add(span().css("mdc-list-item__graphic").style("height: auto; align-self: unset;").add(icon))
			 .add(div().css("mdc-list-item__text").style("margin-bottom: 10px;").add(title)
					   .add(info.add("발견된 변이 정보를 토대로 DGS 검사의 임상소견을 작성합니다.")))
			 .add(meta);
	}
	public void update() {
		...
		InterpretationApi.interpretation(sample, service).then(reports->{
			this.update((Dgs)reports);
			return null;
		});
	}

	private void update(Dgs dto) {
		meta.element().innerHTML = "";
	...
	@Override
	public DgsCollapseElement that() {
		return this;
	}
