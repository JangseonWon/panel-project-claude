...
public class GenePlusExpandElement extends HTMLElementBuilder<HTMLDivElement, GenePlusExpandElement> implements ExpandElement<HTMLDivElement> {
	...
	private static final List<String> variantClassReportable = Arrays.asList("PV", "LPV", "VUS", "P", "LP");
	private final HTMLContainerBuilder<HTMLDivElement> _this;
	private final ReportElement elemReport = new ReportElement(div());
	private final AddendumElement elemAddendum = new AddendumElement(div());
	private final HTMLContainerBuilder<HTMLDivElement> controller = div().css("controller");
	private final ButtonElementToggle btnAddendum = ButtonElement.toggle().css("button").before(IconElement.icon("post_add")).text("Addendum");
	...
	private PanelTest result;
	private GenePlusExpandElement(HTMLContainerBuilder<HTMLDivElement> e, String id, long sample, String service) {
		super(e);
		this.id = id;
		this.sample = sample;
	...
	private Map<String, Boolean> proven = null;
	private void update(PanelTest dto, Map<String, Boolean> proven) {
		this.result = dto;
		this.proven = proven;
	...
	private final class ReportElement extends HTMLElementBuilder<HTMLDivElement, ReportElement> {
		...
		private final TextFieldElement.TextFieldOutlined<String> iptResultText = TextFieldElement.textBox().outlined().css("input").text("Result(Text)").style("width: calc(100% - 500px);");
	...
	private final class AddendumElement extends HTMLElementBuilder<HTMLDivElement, AddendumElement> {
		...
		public AddendumElement update(PanelTest addendum, Map<String, Boolean> proven) {
			if(addendum == null) {
				...
				if(addendum.variants()!=null) variants.update(addendum.variants(), proven);
				else variants.update(new PanelTest.Variant[0], proven);
