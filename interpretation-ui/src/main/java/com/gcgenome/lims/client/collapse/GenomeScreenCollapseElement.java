...
public class GenomeScreenCollapseElement extends HTMLElementBuilder<HTMLDivElement, GenomeScreenCollapseElement> implements CollapseElement<HTMLDivElement> {
	public static GenomeScreenCollapseElement build(String id, long sample, String service) {
		return new GenomeScreenCollapseElement(div(), id, sample, service);
	}
	...
	private final TextFieldElement.TextFieldOutlined<String> summary = TextFieldElement.textBox().outlined().readOnly(true).text("Interpretation").style("width: 100%; margin-bottom: 16px;");
	private final HTMLContainerBuilder<HTMLDivElement> _this;
	private final TestInfo test;
	private final String id;
	private final long sample;
	private final String service;
	private GenomeScreenCollapseElement(HTMLContainerBuilder<HTMLDivElement> e, String id, long sample, String service) {
	...
	private void layout() {
		_this.add(span().css("mdc-list-item__graphic").style("height: auto; align-self: unset;").add(icon))
			 .add(div().css("mdc-list-item__text").style("margin-bottom: 10px;").add(title)
					   .add(info.add("발견된 변이 정보를 토대로 " + test.name() + "의 임상소견을 작성합니다.")))
			 .add(meta);
	}
	public void update() {
		...
		InterpretationApi.interpretation(sample, service).then(reports->{
			this.update((GenomeScreen)reports);
			return null;
		});
	}
	...
	private void update(GenomeScreen dto) {
		...
		if(dto!=null && dto.interpretation()!=null) {
			...
			if(summary!=null) summary = summary.replace("\r", "").replace("\n", "");
			meta.add(this.summary.value(summary)).style("flex-basis: 100%;");
			Message msg = Message.builder().id(id).type(Message.MessageType.COLLAPSE).param("134px").build();
	...
	@Override
	public GenomeScreenCollapseElement that() {
		return this;
	}
