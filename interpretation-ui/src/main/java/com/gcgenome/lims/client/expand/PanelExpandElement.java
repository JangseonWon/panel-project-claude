...
public class PanelExpandElement extends HTMLElementBuilder<HTMLDivElement, PanelExpandElement> implements ExpandElement<HTMLDivElement> {
	...
	protected final TextFieldElement.TextFieldOutlined<Double> iptDepth = TextFieldElement.numberBox().outlined().required(true).css("input").text("Mean depth of coverage").style("width: 300px;");
	protected final TextFieldElement.TextFieldOutlined<Double> iptCoverage = TextFieldElement.numberBox().outlined().required(true).css("input").text("% of Target bases > 10X").style("width: 300px;");
	...
	protected PanelExpandElement(HTMLContainerBuilder<HTMLDivElement> e, String id, long sample, String service) {
		super(e);
		this.id = id;
		this.sample = sample;
			  ...
			  .then(snvs->{
				  ...
				  InterpretationApi.interpretation(sample, service)
								   .then(obj->{
									   ...
									   if(dto == null) {
										   dto = initialValue(coreInSnv, addendumInSnv).reasonForReferral(test.referralDefault());
										   AnalysisApi.analysis(sample)
													  .then(as->{
														  if(as != null && as.length > 0) {
															  Analysis2 analysis = Arrays.stream(as).max(Comparator.comparing(Analysis2::batch)).get();
															  update(analysis);
														  }
														  return null;
													  });
	...
	public void save() {
		...
		if(result.resultText() == null || result.resultText().trim().isEmpty() || result.interpretation() == null || result.interpretation().trim().isEmpty()) return;
	...
	protected final class ReportElement extends HTMLElementBuilder<HTMLDivElement, ReportElement> {
		...
		private final TextFieldElement.TextFieldOutlined<String> iptResultText = TextFieldElement.textBox().outlined().required(true).css("input").text("Result(Text)").style("width: calc(100% - 500px);");
		...
		private final TextAreaElement<String> iptInterpretation = TextAreaElement.textBox().outlined().required(true).style("width: -webkit-fill-available; height: 250px; margin: 16px;").text("Interpretation");
		private final HTMLContainerBuilder<HTMLDivElement> _this;
