...
public class GenomeScreenCollapseElement extends HTMLElementBuilder<HTMLDivElement, GenomeScreenCollapseElement> implements CollapseElement<HTMLDivElement> {
	...
	private final HTMLContainerBuilder<HTMLTableElement> summary = table().style("text-align: right; font-size: var(--mdc-typography-caption-font-size, 0.75rem);")
																		...
																		.add(tr().add(td().style("color: var(--mdc-theme-text-secondary-on-background, rgba(0, 0, 0, 0.54));")
					.add("Tier1 Depth: ")).add(td().add(depth)))
			.add(tr().add(td().style("color: var(--mdc-theme-text-secondary-on-background, rgba(0, 0, 0, 0.54));")
					.add("Tier1 Coverage(10X): ")).add(td().add(x10)));
	...
	private void update(Analysis2 dto) {
		...
		if(dto!=null) {
			...
			if(dto.get("mean depth tier1(x)")!=null) {
				Double v = Double.parseDouble(dto.get("mean depth tier1(x)"));
			...
			if(dto.get("% bases above 10 tier1")!=null) {
				Double v = Double.parseDouble(dto.get("% bases above 10 tier1"));
		...
		Message msg = Message.builder().id(id).type(Message.MessageType.COLLAPSE).param("74px").build();
